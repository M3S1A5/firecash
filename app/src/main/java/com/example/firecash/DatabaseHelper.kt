...
class DatabaseHelper(context: Context) : SQLiteOpenHelper(context, DATABASE_NAME, null, DATABASE_VERSION) {
    ...
    fun obtenerProductoPorNombre(nombre: String): Producto? {
        ...
        val cursor = db.rawQuery("SELECT * FROM $TABLE_PRODUCTOS WHERE $COLUMN_PRODUCTO_NOMBRE=?", arrayOf(nombre))
        if (cursor.moveToFirst()) {
            val nombreProducto = cursor.getString(cursor.getColumnIndexOrThrow(COLUMN_PRODUCTO_NOMBRE))
            val precio = cursor.getDouble(cursor.getColumnIndexOrThrow(COLUMN_PRODUCTO_PRECIO))
            producto = Producto(nombreProducto, precio, 1) // Aquí asumo una cantidad inicial de 1, puedes ajustarlo según tu lógica.
        }
        cursor.close()
        db.close()
        return producto
    ...
    fun obtenerVentasEntreFechas(fechaInicio: String, fechaFin: String): List<Venta> {
        val ventas = mutableListOf<Venta>()
        val db = this.readableDatabase
        val cursor = db.rawQuery("SELECT * FROM $TABLE_VENTAS WHERE $COLUMN_VENTA_FECHA BETWEEN ? AND ?", arrayOf(fechaInicio, fechaFin))
        if (cursor.moveToFirst()) {
            do {
                val fecha = cursor.getString(cursor.getColumnIndexOrThrow(COLUMN_VENTA_FECHA))
                val metodoPago = cursor.getString(cursor.getColumnIndexOrThrow(COLUMN_VENTA_METODO_PAGO))
                val total = cursor.getDouble(cursor.getColumnIndexOrThrow(COLUMN_VENTA_TOTAL))
                ventas.add(Venta(fecha, metodoPago, total))
            } while (cursor.moveToNext())
        }
        cursor.close()
        db.close()
...
data class Venta(val fecha: String, val metodoPago: String, val total: Double)
